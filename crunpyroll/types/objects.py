...
class ObjectsQuery(Object):
    ...
    def parse(cls, response: dict[Any, Any]):
        ...
        if item_type == ContentType.SEASON.value:
            item["season_metadata"]["series_id"] = item["season_metadata"][
                "identifier"
            ].split("|")[0]
            item["season_metadata"]["season_number"] = int(
                item["season_metadata"]["season_display_number"]
            )
            return Season.parse(item)
        if item_type == ContentType.EPISODE.value:
            return Episode.parse(item)
        if item_type == ContentType.MOVIE.value:
